            ...
            db.collection('buyersData')
              ...
              .then((querySnapshot) => {
                if (querySnapshot.empty) {
                    //If there is no request like this in buyers, upload a request to sellers

                    let docName = requestData.book.substring(0, 4) + "_" + requestData.grade.slice(requestData.grade.length - 2) + "%" + requestData.uid;
                    db.collection("sellersData").doc(docName).set(requestData)
                    .then(() => {
...
document.querySelector(".backArrow").addEventListener("click", () => {
  ...
  listItems = document.querySelectorAll(".requestItem");
  listItemClick()
})
...
function listItemClick() {
    listItems.forEach(item => {
        item.addEventListener("click", () => {
          ...
          console.log(docName)
          db.collection("buyersData").where(firebase.firestore.FieldPath.documentId(), '==', docName)
          .get()
          .then((querySnapshot) => {
            if (!querySnapshot.empty) {
            querySnapshot.forEach((doc) => {
